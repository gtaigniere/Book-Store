...
class Form
{
     ...
     * @param string $classLabel Pour ajouter des classes CSS au label
     * @return string
     */
    public function input(string $name, ?string $label = null, array $options = [], string $classLabel = ''): string
    {
        $class = '';
        if ($classLabel != '') {
        ...
        $html = '';
        if ($label != null) {
            $html = '<label' . $class . ' for="' . $name . '">' . $label . '</label>';
        }
        return $html .= '<input id="' . $name . $id . '" name="' . $name . '"' . $params . '>';
    }
